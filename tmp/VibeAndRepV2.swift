// ─────────────────────────────────────────────────────────────────────────────
// VibeAndRepV2.swift  —  NETR App
//
// Vibe Score  — color aura based on how teammates perceive you after games.
//               No numbers. Just a vibe. Green / Yellow / Orange / Red.
//
// Court Rep   — XP system. Play. Rate. Show up. Unlock badges.
//               Think: video game progression, not a leaderboard.
//
// Drop into Rork. No external dependencies. All models + views self-contained.
// ─────────────────────────────────────────────────────────────────────────────
...
// MARK: Vibe Response Options

/// The single post-game question: "How was this player's vibe?"
enum VibeResponse: String, CaseIterable, Identifiable {
    ...
    /// Weight for calculating aura — heavier ends pull harder
    var weight: Double {
        switch self {
...
struct VibeAura {
    ...
    let minScore: Double   // -1.0 to 1.0 weighted average
...
// MARK: Vibe Score Model

struct VibeData {
    let totalResponses: Int
    let counts: [VibeResponse: Int]   // raw response counts

    var isPending: Bool { totalResponses < 5 }

    /// Weighted average from -1.0 to 1.0
...
// MARK: Post-Game Vibe Question View

/// Shown after a player finishes rating skills — one final question.
    ...
    var body: some View {
        VStack(spacing: 0) {
                ...
                HStack(spacing: 10) {
                    ZStack {
                        ...
                        Text(playerInitials)
                            ...
                            .foregroundColor(NC.accent)
                    }
                    Text(playerName)
                        .font(.system(size: 15, weight: .semibold))
                        .foregroundColor(NC.text)
                }
            ...
            Button(action: {
                ...
                .foregroundColor(selected != nil ? .white : NC.sub)
                .cornerRadius(14)
        ...
        Button(action: onTap) {
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(response.label)
                        .font(.system(size: 15, weight: .semibold))
                        .foregroundColor(isSelected ? response.color : NC.text)
...
// MARK: Vibe Aura Display (Profile)

/// The main vibe element on a profile — a glowing color orb + label
...
// MARK: Vibe Detail Sheet (tappable from profile)
        ...
        NavigationView {
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    VStack(spacing: 16) {
                        ...
                        if !vibe.isPending {
                            Text("\(vibe.totalResponses) players weighed in")
                                ...
                                .foregroundColor(aura.color.opacity(0.8))
                    ...
                    VStack(spacing: 10) {
                        VibeInfoRow(icon:"🎯", text:"After rating your skills, teammates answer one question: \"How was this player's vibe?\"")
    ...
    var body: some View {
        ZStack {
            ...
            if aura.label != "PENDING" {
                Text(aura.label)
                    .font(.custom("BarlowCondensed-Black", size: size * 0.2))
                    .foregroundColor(.white.opacity(0.9))
                    .tracking(1.5)
            } else {
                Text("?")
                    .font(.custom("BarlowCondensed-Black", size: size * 0.4))
            ...
            VStack(alignment: .leading, spacing: 5) {
                HStack {
                    Text(response.label)
                        ...
                        .foregroundColor(isDominant ? response.color : NC.text)
                    Spacer()
                    Text("\(count) · \(Int(pct * 100))%")
                        .font(.system(size: 12, weight: .semibold))
                        .foregroundColor(isDominant ? response.color : NC.sub)
                }
...
struct RepBadge: Identifiable {
    ...
    let color: Color
    let xpThreshold: Int   // total XP needed to unlock (0 = action-based)
    let isUnlocked: Bool
}
...
extension RepBadge {
    static func allBadges(for profile: CourtRepData) -> [RepBadge] {[
        ...
        RepBadge(id:"known_face",    name:"Known Face",     icon:"👋", description:"People know you at the courts",           requirement:"20 games played",        color:Color(hex:"#9B6DFF"), xpThreshold:0,   isUnlocked: profile.gamesPlayed >= 20),
...
extension CourtRepData {
    static let mock = CourtRepData(
        ...
        recentActions: [
            ...
            RecentRepAction(id:"3", icon:"📋", label:"Hosted a game",        xp:15,  timeAgo:"2 days ago"),
        ]
    )
}

// MARK: Court Rep Profile Card (profile embed)
                ...
                HStack(spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        ...
                        Text("\(data.totalXP) XP")
                            .font(.system(size: 12, weight: .semibold))
                            .foregroundColor(data.level.color.opacity(0.75))
                    }
                    Spacer()
                    Image(systemName: "chevron.right")
                        .font(.system(size: 12, weight: .semibold))
                        .foregroundColor(NC.muted)
                }
                .padding(.bottom, 14)
    ...
    var body: some View {
        VStack(spacing: 4) {
            ...
            if !compact {
                Text(badge.name)
                    .font(.system(size: 10, weight: .semibold))
                    .foregroundColor(locked ? NC.muted : badge.color)
                    .lineLimit(1)
            }
        }
    }
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 0) {
                // Tab switcher
                HStack(spacing: 0) {
                    ForEach(["Progress", "Badges", "How to Earn"].indices, id: \.self) { i in
                        let labels = ["Progress", "Badges", "How to Earn"]
                        Button(action: { withAnimation { selectedTab = i } }) {
                            Text(labels[i])
                                .font(.system(size: 13, weight: .semibold))
                                .foregroundColor(selectedTab == i ? NC.text : NC.sub)
                                .frame(maxWidth: .infinity)
                                .padding(.vertical, 12)
                ...
                HStack(spacing: 12) {
                    ZStack {
                        ...
                        Text("L\(lvl.level)")
                            ...
                            .foregroundColor(isCurrent || isPast ? lvl.color : NC.muted)
                    }
                    VStack(alignment: .leading, spacing: 2) {
                        Text(lvl.name)
                            .font(.system(size: 14, weight: .semibold))
                            .foregroundColor(isCurrent ? lvl.color : isPast ? NC.sub : NC.muted)
                        Text("\(lvl.minXP)+ XP")
                            .font(.system(size: 11))
                            .foregroundColor(NC.muted)
                    }
                    ...
                    if isCurrent {
                        ...
                        Image(systemName: "checkmark.circle.fill")
                            .foregroundColor(lvl.color.opacity(0.6))
                    }
                }
                .padding(12)
...
// Needed for lineHeight workaround
extension View {
    func lineHeight(_ height: CGFloat) -> some View { self }
}
...
// ─────────────────────────────────────────────────────────────────────────────
// MARK: ═══ COMBINED PROFILE BLOCK ════════════════════════════════════════════
// ─────────────────────────────────────────────────────────────────────────────

/// Drop this into ProfileView below the NETR score
...
#Preview("Vibe Detail — Active") {
    VibeDetailSheet(vibe: .mockGreen)
}

#Preview("Vibe Detail — Pending") {
    VibeDetailSheet(vibe: .mockPending)
}

#Preview("Combined Block") {
    ScrollView {
        VStack(spacing: 20) {
            VibeAndRepBlock(vibe: .mockGreen, rep: .mock)
        }
        .padding(20)
    }
...
#Preview("Court Rep Sheet") {
    CourtRepSheet(data: .mock)
}
